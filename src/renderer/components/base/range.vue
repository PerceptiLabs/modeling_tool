<template>
  <div class="custom-range-component-wrap">
    <span class="custom-range_minmax">{{ min }}</span>
    <div
      ref="wrap"
      :class="['custom-range-component', flowDirection, disabledClass, stateClass, { 'custom-range-has-label': piecewiseLabel }]"
      v-show="show"
      :style="[wrapStyles, boolDisabled ? disabledStyle : null]"
      @click="wrapClick"
    >
      <div ref="elem" aria-hidden="true" class="custom-range" :style="[elemStyles, bgStyle]">
        <template v-if="isRange">
          <div
            ...
            :class="[
              ...
              'custom-range-dot',
              {
                'custom-range-dot-focus': focusFlag && focusSlider === 0,
                'custom-range-dot-dragging': flag && currentSlider === 0,
                'custom-range-dot-disabled': !boolDisabled && disabledArray[0]
              }
            ]"
            :style="dotStyles"
          ...
          >
            ...
            <div ref="tooltip0" :class="['custom-range-tooltip-' + tooltipDirection[0], 'custom-range-tooltip-wrap']">
              <slot name="tooltip" :value="val[0]" :index="0" :disabled="!boolDisabled && disabledArray[0]">
                <span class="custom-range-tooltip" :style="tooltipStyles[0]">{{ formatter ? formatting(val[0]) : val[0] }}</span>
              </slot>
            </div>
          </div>
          <div
            ...
            :class="[
              ...
              'custom-range-dot',
              {
                'custom-range-dot-focus': focusFlag && focusSlider === 1,
                'custom-range-dot-dragging': flag && currentSlider === 1,
                'custom-range-dot-disabled': !boolDisabled && disabledArray[1]
              }
            ]"
            :style="dotStyles"
          ...
          >
            ...
            <div ref="tooltip1" :class="['custom-range-tooltip-' + tooltipDirection[1], 'custom-range-tooltip-wrap']">
              <slot name="tooltip" :value="val[1]" :index="1" :disabled="!boolDisabled && disabledArray[1]">
                <span class="custom-range-tooltip" :style="tooltipStyles[1]">{{ formatter ? formatting(val[1]) : val[1] }}</span>
              </slot>
            </div>
          </div>
        </template>
        <template v-else>
          ...
          >
            ...
            <div :class="['custom-range-tooltip-' + tooltipDirection, 'custom-range-tooltip-wrap']">
              <slot name="tooltip" :value="val">
                <span class="custom-range-tooltip" :style="tooltipStyles">{{ formatter ? formatting(val) : val }}</span>
              </slot>
            </div>
          </div>
        </template>
        <ul class="custom-range-piecewise">
          <li v-for="(piecewiseObj, index) in piecewiseDotWrap" class="custom-range-piecewise-item" :style="[piecewiseDotStyle, piecewiseObj.style]" :key="index">
            <slot
              name="piecewise"
            ...
            >
              <span
                v-if="piecewise"
                class="custom-range-piecewise-dot"
            ...
            >
              <span
                v-if="piecewiseLabel"
                class="custom-range-piecewise-label"
        ...
        <div
          ref="process"
          :class="['custom-range-process', { 'custom-range-process-dragable': isRange && processDragable }]"
          :style="processStyle"
          @click="processClick"
        ...
        <div
          ref="mergedTooltip"
          :class="['vue-merged-tooltip', 'custom-range-tooltip-' + tooltipDirection[0], 'custom-range-tooltip-wrap']"
          :style="tooltipMergedPosition"
        >
            <slot name="tooltip" :value="val" :merge="true">
              <span class="custom-range-tooltip" :style="tooltipStyles">
      ...
      <input v-if="!isRange && !data" class="custom-range-sr-only" type="range" v-model="val" :min="min" :max="max" />
      </div>
    </div>
    <span class="custom-range_minmax">{{ max }}</span>
  </div>
</template>
<script>
  // Unsharp text [#166](https://github.com/NightCatSama/custom-range-component/issues/166)
  ...
  export default {
    ...
    computed: {
      ...
      flowDirection () {
        return `custom-range-${this.direction + (this.reverse ? '-reverse' : '')}`
      },
      ...
      tooltipStatus () {
        return this.tooltip === 'hover' && this.flag ? 'custom-range-always' : this.tooltip ? `custom-range-${this.tooltip}` : ''
      },
      tooltipClass () {
        return [`custom-range-tooltip-${this.tooltipDirection}`, 'custom-range-tooltip']
      },
      disabledArray () {
      ...
      disabledClass () {
        return this.boolDisabled ? 'custom-range-disabled' : ''
      },
      stateClass () {
        return {
          'custom-range-state-process-drag': this.processFlag,
          'custom-range-state-drag': this.flag && !this.processFlag && !this.keydownFlag,
          'custom-range-state-focus': this.focusFlag
        }
      },
      isRange () {
...
<style lang="scss">
  ...
  .custom-range-component.custom-range-disabled {
    opacity: .5;
    cursor: not-allowed;
  }
  .custom-range-component.custom-range-has-label {
    margin-bottom: 15px;
  }
  .custom-range-component.custom-range-disabled .custom-range-dot {
    cursor: not-allowed;
  }
  .custom-range-component .custom-range {
    position: relative;
    display: block;
    border-radius: 15px;
  ...
  .custom-range-component .custom-range-dot.custom-range-dot-focus .custom-range-dot-handle {
    //box-shadow: 0 0 2px 1px #3498db;
  }
  .custom-range-component .custom-range-dot.custom-range-dot-dragging {
    z-index: 5;
  }
  .custom-range-component .custom-range-dot.custom-range-dot-disabled {
    z-index: 4;
  }
  .custom-range-component.custom-range-horizontal .custom-range-dot {
    left: 0;
  }
  .custom-range-component.custom-range-vertical .custom-range-dot {
    bottom: 0;
  }
  .custom-range-component.custom-range-horizontal-reverse .custom-range-dot {
    right: 0;
  }
  .custom-range-component.custom-range-vertical-reverse .custom-range-dot {
    top: 0;
  }
  ...
  .custom-range-component .custom-range-tooltip-wrap.custom-range-tooltip-bottom {
    bottom: -9px;
    left: 50%;
    transform: translate(-50%, 100%);
  }
  .custom-range-component .custom-range-tooltip-wrap.custom-range-tooltip-left {
    top: 50%;
    left: -9px;
    transform: translate(-100%, -50%);
  }
  .custom-range-component .custom-range-tooltip-wrap.custom-range-tooltip-right {
    top: 50%;
    right: -9px;
    transform: translate(100%, -50%);
  }
  .custom-range-component .custom-range-tooltip-wrap.custom-range-tooltip-top .custom-range-tooltip::before,
  .custom-range-component .custom-range-tooltip-top .vue-merged-tooltip .custom-range-tooltip::before {
    content: '';
    position: absolute;
    bottom: -10px;
    left: 50%;
  ...
  .custom-range-component .custom-range-tooltip-wrap.custom-range-tooltip-bottom .custom-range-tooltip::before,
  .custom-range-component .custom-range-tooltip-bottom .vue-merged-tooltip .custom-range-tooltip::before {
    content: '';
    position: absolute;
    top: -10px;
    left: 50%;
  ...
  .custom-range-component .custom-range-dot.custom-range-always .custom-range-tooltip-wrap {
