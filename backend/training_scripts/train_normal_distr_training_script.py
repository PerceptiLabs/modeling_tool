...
from perceptilabs.core_new.layers.replication import (
    BASE_TO_REPLICA_MAP,
...
class DataData_data_inputs(DataLayer):
    ...
    def __init__(self):
        ...
        (
            trn_gens_args_DataData_data_inputs,
            val_gens_args_DataData_data_inputs,
            tst_gens_args_DataData_data_inputs,
        ) = ([], [], [])

        columns_DataData_data_inputs_0 = None

        global matrix_DataData_data_inputs_0
        matrix_DataData_data_inputs_0 = np.load(
            "C:/Users/Robert/AppData/Local/Temp/tmpqytsc7ls.npy"
        ).astype(np.float32)
        ...
        trn_gens_args_DataData_data_inputs.append(
            (generator_DataData_data_inputs_0, 0, trn_sz)
        )
        val_gens_args_DataData_data_inputs.append(
            (generator_DataData_data_inputs_0, trn_sz, trn_sz + val_sz)
        )
        tst_gens_args_DataData_data_inputs.append(
            (
                generator_DataData_data_inputs_0,
                trn_sz + val_sz,
                trn_sz + val_sz + tst_sz,
            )
        )
...
class DataData_data_labels(DataLayer):
    ...
    def __init__(self):
        ...
        trn_sz_tot, val_sz_tot, tst_sz_tot = 0, 0, 0
        (
            trn_gens_args_DataData_data_labels,
            val_gens_args_DataData_data_labels,
            tst_gens_args_DataData_data_labels,
        ) = ([], [], [])

        columns_DataData_data_labels_0 = None

        global matrix_DataData_data_labels_0
        matrix_DataData_data_labels_0 = np.load(
            "C:/Users/Robert/AppData/Local/Temp/tmpv0nqf7sn.npy"
        ).astype(np.float32)
        ...
        trn_gens_args_DataData_data_labels.append(
            (generator_DataData_data_labels_0, 0, trn_sz)
        )
        val_gens_args_DataData_data_labels.append(
            (generator_DataData_data_labels_0, trn_sz, trn_sz + val_sz)
        )
        tst_gens_args_DataData_data_labels.append(
            (
                generator_DataData_data_labels_0,
                trn_sz + val_sz,
                trn_sz + val_sz + tst_sz,
            )
        )
    ...
    def __call__(self, x: tf.Tensor):
        ...
        with tf.compat.v1.variable_scope(self._scope, reuse=tf.compat.v1.AUTO_REUSE):
            initial = tf.random.truncated_normal(
                (n_inputs, self._n_neurons), stddev=0.1
            )
...
class TrainNormal_training(ClassificationLayer):
    ...
    def run(self, graph: Graph):
        ...
        config = tf.ConfigProto(
            device_count={"CPU": n_devices, "GPU": 0},
        ...
        strategy = tf.distribute.MirroredStrategy(
            devices=[f"/CPU:{i}" for i in range(n_devices)]
        )  # TODO: not needed under real circumstances, should default to all.

        BATCH_SIZE_PER_REPLICA = (
            self._batch_size
        )  # TODO: should this be batch_size divided by n_devices or not?
        ...
        test_dataset = dataset_tst.batch(
            1
        )  # Since the batch size for test is 1, it does not make sense to divide the batch over several replicas. Do testing as usual.
        ...
        with strategy.scope():
            ...
            if INCLUDE_KERAS_METRICS:
                ...
                f1_train = tf.reduce_max(
                    tf.math.divide_no_nan(2 * r * p, r + p)
                )  # TODO: create custom metric instead? make PR at tf?
            ...
            def train_step(inputs):
                ...
                loss_tensor = (
                    tf.reduce_sum(tf.square(output_tensor - target_tensor))
                    / GLOBAL_BATCH_SIZE
                )
                correct_predictions = tf.equal(
                    tf.argmax(output_tensor, -1), tf.argmax(target_tensor, -1)
                )
                ...
                for node in graph.inner_nodes:
                    if not isinstance(
                        node.layer, Tf1xLayer
                    ):  # In case of pure custom layers...
                        continue
                    ...
                    if len(node.layer.trainable_variables) > 0:
                        ...
                        self._layer_gradients[node.layer_id] = {
                            name: [] for name in node.layer.trainable_variables.keys()
                        }  # Initialize

                trainable_vars = tf.trainable_variables()
                grads = tf.gradients(loss_tensor, trainable_vars)
                update_weights = optimizer.apply_gradients(
                    zip(grads, trainable_vars), global_step=global_step
                )

                if INCLUDE_KERAS_METRICS:
                    update_auc = auc_train.update_state(target_tensor, output_tensor)
                    update_recall = recall_train.update_state(
                        target_tensor, output_tensor
                    )
                    update_precision = precision_train.update_state(
                        target_tensor, output_tensor
                    )
                ...
                with tf.control_dependencies(update_ops):
                    ...
                    return (
                        ...
                        add_identity(accuracy_tensor),
                        add_identity(layer_output_tensors),
                        add_identity(layer_weight_tensors),
                        add_identity(layer_bias_tensors),
                        add_identity(layer_gradient_tensors),
                    )
            ...
            if n_devices > 1:
                ...
                dist_loss_train = [
                    dist_loss_train.get(device) for device in dist_loss_train.devices
                ]
                ...
                (
                    ...
                    layer_biases_val,
                    layer_gradients_val,
                ) = strategy.experimental_run(validation_step, validation_iterator)
                ...
                layer_gradients_val = {
                    k: v for k, v in layer_gradients_val.items() if v is not None
                }
                ...
                self._input_tensor_export = tf.placeholder(
                    shape=[None] + dataset_trn.output_shapes[0].as_list(),
                    dtype=dataset_trn.output_types[0],
                )

                self._output_tensor_export = model(
                    self._input_tensor_export,
                    tf.placeholder(
                        shape=[None] + dataset_trn.output_shapes[1].as_list(),
                        dtype=dataset_trn.output_types[1],
                    ),
                )[output_layer_id]
            else:
            ...
            if INCLUDE_KERAS_METRICS:
                sess.run(
                    [v.initializer for v in auc_train.variables]
                )  # these need spec. treatment when initializing
    ...
    def on_export(self, path: str, mode: str) -> None:
        ...
        if mode in ["TFModel", "TFModel+checkpoint"]:
            tf.compat.v1.saved_model.simple_save(
                self._sess,
                pb_path,
                inputs={"input": self._input_tensor_export},
                outputs={"output": self._output_tensor_export},
            )

        # Export compressed model
        if mode in ["TFLite", "TFLite+checkpoint"]:
            converter = tf.lite.TFLiteConverter.from_session(
                self._sess, [self._input_tensor_export], [self._output_tensor_export]
            )
        ...
        if mode in ["TFModel+checkpoint", "TFLite+checkpoint"]:
            self._saver.save(
                self._sess, os.path.join(path, "model.ckpt"), global_step=0
            )
...
def endpoint_snapshot():
    ...
    try:
        ...
        non_picklable = boltons.iterutils.research(
            snapshots[index],
            query=lambda p, k, v: type(v) not in [list, dict, set, tuple]
            and not can_serialize(v),
        )
        if non_picklable:
...
snapshot_builder = SnapshotBuilder(BASE_TO_REPLICA_MAP, REPLICATED_PROPERTIES_TABLE)
...
def process_events(graph):
    ...
    while not event_queue.empty():
        ...
        if event_type == "on_pause":
            ...
            graph.active_training_node.layer.on_export(
                event_data["path"], event_data["mode"]
            )
...
def main(wait=False):
    ...
    threading.Thread(
        target=app.run, kwargs={"port": "5680", "threaded": True}, daemon=True
    ).start()
    threading.Thread(target=message_queue_worker, daemon=True).start()
    graph = get_graph()

    print(graph.training_nodes)
    graph.training_nodes[
        0
    ].layer_instance.save_snapshot = make_snapshot_and_process_events
    graph.training_nodes[
        0
    ].layer_instance.save_snapshot_and_process_events = make_snapshot_and_process_events
