    ...
    with builder:
        for i in range(n_samples):
            with builder.create_row() as row:
                row.file_data["x"] = np.random.randint(
                    0, 255, (28, 28, 1), dtype=np.uint8
                )
                row.file_type["x"] = ".png"
                row.literals["y"] = i
...
def graph_spec():
    ...
    # Create the layers
    id1 = gsb.add_layer(
        "IoInput", settings={"id_": "input0", "datatype": "image", "feature_name": "x"}
    )
    id2 = gsb.add_layer(
        "ProcessReshape", settings={"id_": "reshape0", "shape": (14, 14, 4)}
    )
    id3 = gsb.add_layer("DeepLearningFC", settings={"id_": "fc0", "n_neurons": 1})
    id4 = gsb.add_layer(
        "IoOutput",
    ...
    gsb.add_connection(
        source_id=id3, source_var="output", dest_id=id4, dest_var="input"
    )
...
def graph_spec_pair():
    def create(n_neurons):
        ...
        id2 = gsb.add_layer("DeepLearningFC", settings={"id_": "fc0", "n_neurons": 10})
        id3 = gsb.add_layer(
            "DeepLearningFC", settings={"id_": "fc1", "n_neurons": n_neurons}
        )
        id4 = gsb.add_layer("DeepLearningFC", settings={"id_": "fc2", "n_neurons": 1})
        id5 = gsb.add_layer(
            "IoOutput",
            settings={"id_": "output0", "datatype": "categorical", "feature_name": "y"},
        )
        ...
        gsb.add_connection(
            source_id=id4, source_var="output", dest_id=id5, dest_var="input"
        )
        return gsb.build()
...
def graph_spec_partial():
    ...
    id3 = gsb.add_layer("DeepLearningFC", settings={"id_": "fc0", "n_neurons": 1})
    id4 = gsb.add_layer("DeepLearningFC", settings={"id_": "fc1", "n_neurons": 1})
    id5 = gsb.add_layer(
        "IoOutput",
        settings={"id_": "output0", "datatype": "categorical", "feature_name": "y"},
    )
...
def graph_spec_3d():
    ...
    id2 = gsb.add_layer(
        "ProcessReshape", settings={"id_": "reshape0", "shape": (14, 14, 12)}
    )
    id3 = gsb.add_layer("DeepLearningFC", settings={"id_": "fc0", "n_neurons": 1})
    id4 = gsb.add_layer(
        "IoOutput",
...
def graph_spec_wrong_output_shape():
    gsb = GraphSpecBuilder()
    # Create the layers
    id1 = gsb.add_layer("IoInput", settings={"datatype": "image", "feature_name": "x"})
    id2 = gsb.add_layer("DeepLearningFC", settings={"n_neurons": 11})
    id3 = gsb.add_layer(
...
def graph_spec_runtime_error():
    ...
    id3 = gsb.add_layer("DeepLearningFC", settings={"n_neurons": 1})
    id4 = gsb.add_layer(
        "IoOutput", settings={"datatype": "numerical", "feature_name": "y"}
    )
...
def test_io_layer_samples_are_from_the_same_row(data_loader, graph_spec):
    ...
    def previews_match_row_in_dataset(input_preview, target_preview):
        for inputs_batch, targets_batch in data_loader.get_dataset(
            partition="training"
        ):
            input_value = inputs_batch["x"].numpy()
            target_value = targets_batch["y"].numpy()

            if np.all(input_preview == input_value) and np.all(
                target_preview == target_value
            ):
                return True
...
def test_output_layer_raises_error_if_inputs_shape_doesnt_match(
    data_loader, graph_spec_wrong_output_shape
):
    lw_core = LightweightCore(data_loader)
