...
class GradioStrategy:
    def __init__(
        ...
        self._url_dict = (
            dict()
        )  # multiprocessing.Manager().dict()  # So we can pass url between processes
    ...
    def _worker_internal(
        ...
        inputs = {
            spec.feature_name: self.get_gradio_input(
                spec.feature_name, spec.datatype, metadata[spec.feature_name]
            )
            for spec in graph_spec.input_layers
        }
        targets = {
            spec.feature_name: self.get_gradio_output(
                spec.feature_name,
                spec.datatype,
                metadata[spec.feature_name],
                include_preprocessing=include_preprocessing,
                include_postprocessing=include_postprocessing,
            )
        ...
        def fn_inference(*input_values):
            ...
            output_values = self._create_output_values(
                y,
                targets,
                metadata,
                graph_spec,
        ...
        examples = self._get_gradio_examples(
        ...
        flask_app, path_to_local_server, share_url = interface.launch(
            share=False, prevent_thread_lock=True
        )  # Warning: Gradio conflicts with our Flask development server. URL is only valid when we run the kernel with debug == False
    ...
    def _create_output_values(
        ...
        if len(targets) == 1:
            ...
            output_values = [
                model_output[name].squeeze().tolist() for name in targets.keys()
            ]

        dtype = self._get_dtype(graph_spec.target_layers)
        if not include_postprocessing and dtype == "categorical":
            output_values = self._create_categorical_output(
                model_output, metadata, graph_spec
            )
    ...
    def _create_categorical_output(self, model_output, metadata, graph_spec):
        categories = self._get_all_categories(
            metadata=metadata, target_layers=graph_spec.target_layers
        )
        output_values = dict()
    ...
    def _get_all_categories(self, metadata, target_layers):
        if len(target_layers) > 1:
            raise NotImplementedError(
                "Can't get categories when there is more than one target layer!"
            )
        for spec in target_layers:
            categories = list(
                metadata[spec.feature_name]["preprocessing"]["mapping"].keys()
            )
            return categories

    def _get_gradio_examples(
        self, data_loader, include_preprocessing, include_postprocessing
    ):
        ...
        if len(test_set_df) > 0:
            ...
            examples = ServingResultsAccess.create_dir_for_gradio_examples(
                samples=samples
            )  # A list of paths
            return examples
    ...
    def get_gradio_input(feature_name, datatype, metadata):
        ...
        if datatype == "categorical":
            return gr.inputs.Radio(
                choices=list(metadata["preprocessing"]["mapping"].keys()),
                label=feature_name,
            )
        elif datatype == "text":
            ...
            if n_channels == 3:
                ...
                raise ValueError(
                    f"Unexpected number of channels in metadata: {n_channels}"
                )

            return gr.inputs.Image(
                image_mode=image_mode, label=feature_name, type="file"
            )
        else:
            raise NotImplementedError(
                f"No gradio input type found for datatype '{datatype}'"
            )

    @staticmethod
    def get_gradio_output(
        feature_name,
        datatype,
        metadata,
        include_preprocessing=True,
        ...
        if datatype == "numerical":
            ...
            raise NotImplementedError(
                f"No gradio output type found for datatype '{datatype}'"
            )
