...
def create_macro_loader(macro_file, macro_name, macro_parameters):
    ...
    arg_str = ", ".join(
        f"{arg_name}={arg_value}" for arg_name, arg_value in _macro_params.items()
    )

    template = _RENDER_MACRO_TEMPLATE.format(
        macro_file=macro_file, macro_name=macro_name, arg_str=arg_str
    ).strip()

    return template


def render_and_execute_macro(
    j2_engine, macro_file, macro_name, macro_parameters, import_statements=None
):
...
class MyLoader:
    ...
    def exec_module(self, module):
        ...
        code_obj = compile(
            self._source_bytes, file_name, "exec", dont_inherit=True, optimize=2
        )
