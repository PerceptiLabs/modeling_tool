...
mk_abs = lambda x: pkg_resources.resource_filename(
    "perceptilabs", x
)  # Make absolute path
...
class ScriptFactory:
    def __init__(
        self, max_time_run=None, simple_message_bus=False, running_mode="initializing"
    ):
    ...
    def get_layer_import_statements(self, layer_spec, include_top_level=False):
            ...
            for stmt in from_list:
                if not is_syntax_ok(stmt):
                    raise ScriptBuildError(
                        f"Invalid syntax for import statement '{stmt}' found in layer def for {layer_spec.layer_type}"
                    )
                target_set.add(stmt)
        ...
        if layer_def is None:
            raise RuntimeError(
                f"No layer definition for '{layer_spec.type_}'. Check perceptilabs.layers.definitions module"
            )
        ...
        if os.path.isfile(_imports_path):
            ...
            raise ValueError(
                f"Couldn't locate imports file {_imports_path}. Used directory '.' and '{package_root}'"
            )
        ...
        with open(imports_path, "r") as f:
            try:
                ...
                logger.exception(
                    f"Error when opening imports file at {imports_path}. Layer: {layer_spec}"
                )
                raise
    ...
    def _create_layers_snippet(self, graph_spec, offset=None):
        ...
        for layer_spec in graph_spec.nodes_by_id.values():
            layer_code = self.render_layer_code(
                layer_spec,
                macro_kwargs={"graph_spec": graph_spec, "layer_spec": layer_spec},
            )
            code += layer_code + "\n"
            n_lines = len(layer_code.split("\n"))

            if offset is not None:
                line_to_node_map.update(
                    {offset + line: (layer_spec, line) for line in range(n_lines)}
                )
                offset += n_lines
    ...
    def _create_training_server_snippet(
        self, topic_generic, topic_snapshots, userland_timeout
    ):
    ...
    def make(
        ...
        layers_code, line_to_node_map = self._create_layers_snippet(
            graph_spec, initial_offset
        )
        code += layers_code

        code += self._create_graph_snippet(graph_spec)
        code += self._create_training_server_snippet(
            topic_generic, topic_snapshots, userland_timeout
        )
    ...
    def render_layer_code(self, layer_spec, macro_kwargs=None):
        if layer_spec.custom_code is not None:
            ...
            if os.path.isfile(_macro_path):
                ...
                raise ValueError(
                    f"Couldn't locate macro {_macro_path}. Used directory '.' and '{package_root}'"
                )
            ...
            try:
                ...
                logger.exception(
                    f"Error when rendering jinja macro {layer_def.macro_path}:{layer_def.macro_name}. Contents :\n"
