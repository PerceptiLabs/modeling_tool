...
class IouStats:
    ...
    def get_average_iou_for_epoch(self, epoch, phase="training"):
        ...
        for matrix, is_training in self.prediction_matrices[epoch]:
            if (is_training and phase in ["both", "training"]) or (
                ...
                intersection += (
                    matrix.tp
                )  # intersection: both y_pred and y_true are positive
                union += (
                    matrix.tp + matrix.fp + matrix.fn
                )  # union: y_pred or y_true is positive
    ...
    def get_iou_over_steps_in_latest_epoch(self, phase="training"):
        ...
        return self.get_iou_over_steps(
            epoch=len(self.prediction_matrices) - 1, phase=phase
        )
...
class IouStatsTracker(TrainingStatsTracker):
    ...
    def _store_prediction_matrix(
        ...
        thresholded_batch = tf.where(
            predictions_batch >= threshold, 1.0, 0.0
        )  # Convert to positive(1.0)/negative(0.0) so we can compute the confusion matrix
