...
def test_get_loss_over_steps_training(losses):
    ...
    for epoch, epoch_losses in enumerate(losses):
        expected = [loss for loss, is_training in losses[epoch] if is_training]
...
def test_get_loss_over_steps_validation(losses):
    ...
    for epoch, epoch_losses in enumerate(losses):
        expected = [loss for loss, is_training in losses[epoch] if not is_training]
...
def test_get_average_loss_for_epoch_training(losses):
    ...
    for epoch, epoch_losses in enumerate(losses):
        expected = np.average(
            [loss for loss, is_training in losses[epoch] if is_training]
        )
...
def test_get_average_loss_for_epoch_validation(losses):
    ...
    for epoch, epoch_losses in enumerate(losses):
        expected = np.average(
            [loss for loss, is_training in losses[epoch] if not is_training]
        )
...
def test_trackers_are_equal_when_both_are_updated():
    ...
    tracker1.update(
        loss=tf.constant(123.0), epochs_completed=0, steps_completed=0, is_training=True
    )
    assert tracker1 != tracker2
    assert tracker1.save() != tracker2.save()
