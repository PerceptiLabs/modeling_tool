...
class MultiClassMatrixStats:
    ...
    def get_precision_over_epochs(self, phase="training"):
        ...
        num_epochs = (
            len(summed_matrices)
            if sum(sum(np.array(summed_matrices[-1]))) > 0
            else len(summed_matrices) - 1
        )
    ...
    def get_f1_over_epochs(self, precision_over_epochs, recall_over_epochs):
        ...
        for index, _ in np.ndenumerate(f1_over_epochs):
            if precision_over_epochs[index] == 0 or recall_over_epochs[index] == 0:
                f1_over_epochs[index] = 0.0
            else:
                f1_over_epochs[index] = (
                    2
                    * precision_over_epochs[index]
                    * recall_over_epochs[index]
                    / (recall_over_epochs[index] + precision_over_epochs[index])
                )
    ...
    def get_total_matrices_for_all_epochs(self, phase="training"):
        ...
        for epoch in range(len(self.prediction_matrices)):
            summed_matrix = self.get_total_matrix_for_given_epoch(
    ...
    def get_total_matrix_for_given_epoch(self, epoch=0, phase="training"):
        ...
        summed_matrix = [
            [0 for _ in range(len(matrices[0][0].prediction_matrix))]
            for _ in range(len(matrices[0][0].prediction_matrix))
        ]
...
class MultiClassMatrixStatsTracker(TrainingStatsTracker):
    ...
    def _store_prediction_matrix(
        ...
        if len(self._prediction_matrices) <= epochs_completed:
            self._prediction_matrices.append(
                list()
            )  # Create list to hold steps for epoch.

        self._num_categories = targets_batch.shape[-1]
        prediction_matrix = [
            [0 for _ in range(self._num_categories)]
            for _ in range(self._num_categories)
        ]
        ...
        if (
            isinstance(target_value_indices, list)
            and isinstance(predicted_value_indices, list)
        ) or (
            isinstance(target_value_indices, np.ndarray)
            and isinstance(predicted_value_indices, np.ndarray)
        ):
            for (target_value_index, pred_value_index) in zip(
                target_value_indices, predicted_value_indices
            ):
    ...
    def save(self):
        ...
        pred_matrices = tuple(
            [tuple(step_matrices) for step_matrices in self._prediction_matrices]
        )
