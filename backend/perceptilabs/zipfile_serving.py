...
class ZipfileStrategy:
    ...
    def start(self):
        if self._export_settings["Type"] == "PlPackage":
            ...
            for relative_path in self._export_non_archive():
                file_path = os.path.join(self._export_directory, relative_path).replace(
                    "\\", "/"
                )
        ...
        logger.info(f"Model archive write called for path {self._zipfile_path}")
    ...
    def _export_non_archive(self):
        ...
        exporter = Exporter(self._graph_spec, training_model, self._data_loader)
