...
@pytest.mark.parametrize("batch_size", [1, 8])
def test_inference_model_can_predict_with_loaded_preprocessed_and_postprocessed_data(
    script_factory, data_loader, batch_size
):
    x, y_true = data_loader.get_example_batch(
        batch_size=batch_size, apply_pipelines="all"  # load and preprocess
    )

    graph_spec = make_graph_spec(data_loader)
    inference_model = TrainingModel(script_factory, graph_spec).as_inference_model(
        data_loader,
...
@pytest.mark.parametrize("batch_size", [1, 8])
def test_inference_model_can_predict_with_loaded_but_neither_preprocessed_data_nor_postprocessed_data(
    script_factory, data_loader, batch_size
):
    x, y_true = data_loader.get_example_batch(
...
@pytest.mark.parametrize("batch_size", [1, 8])
def test_inference_model_can_predict_with_loaded_and_postprocessed_but_not_preprocessed_data(
    script_factory, data_loader, batch_size
):
    x, y_true = data_loader.get_example_batch(
        batch_size=batch_size, apply_pipelines="loader"
...
@pytest.mark.parametrize("batch_size", [1, 8])
def test_inference_model_can_predict_with_loaded_and_preprocessed_and_postprocessed_data(
    script_factory, data_loader, batch_size
):
    x, y_true = data_loader.get_example_batch(
