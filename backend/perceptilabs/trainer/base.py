...
from perceptilabs.stats import (
    SampleStatsTracker,
    SampleStats,
...
from perceptilabs.layers.iooutput.stats import (
...
class Trainer:
    def __init__(
        ...
        on_training_stopped=None,
        on_training_completed=None,
        on_epoch_completed=None,
    ):
        ...
        self._hardware_stats = HardwareStats(
            refresh_interval=settings.TRAINING_RESULTS_REFRESH_INTERVAL
        )  # Worst case scenario, it is a factor of 2 seconds behind.
    ...
    def _initialize_results(self):
        ...
        if self._initial_state is None:
            ...
            self._num_batches_completed_all_epochs = initial_state[
                "num_batches_completed_all_epochs"
            ]
            ...
            self._num_epochs = (
                int(self._training_settings["Epochs"])
                + initial_state["num_epochs_completed"]
            )
    ...
    def ensure_data_initialized(self):
        ...
        self._num_batches_per_epoch = (
            training_batches_per_epoch + validation_batches_per_epoch
        )
    ...
    def save_state(self):
        state = {
            ...
            "output_trackers": {
                layer_id: tracker for layer_id, tracker in self._output_trackers.items()
            },
            "input_trackers": {
                layer_id: tracker for layer_id, tracker in self._input_trackers.items()
            },
    ...
    def run_stepwise(self):
        ...
        if self._num_epochs_completed == 0:
            ...
            logger.info(
                f"Entering training loop (starting from epoch {self._num_epochs_completed}/{self.num_epochs})"
            )
        ...
        if self._on_training_completed:
            (
                ...
                model_params,
                trainable_params,
            ) = self._get_mixpanel_pricing_metrics()
    ...
    def _loop_over_dataset(
        self,
        model,
        loss_functions,
        dataset,
        set_num_batches_completed_this_epoch,
        is_closed,
        is_training=True,
        optimizer=None,
    ):
    ...
    @tf.function
    def _work_on_batch(
        self, model, loss_functions, inputs_batch, targets_batch, is_training, optimizer
    ):
        ...
        return (
            ...
            trainables_by_layer,
            gradients_by_layer,
            final_and_intermediate_outputs,
            total_loss,
            individual_losses,
        )

    def _compute_gradients(
        self, tape, total_loss, trainables_by_layer, flat_trainables
    ):
    ...
    def _reset_tracked_values(self, initial_state=None):
        if initial_state is None:
            ...
            self._inner_layers_stats_tracker = initial_state[
                "inner_layers_stats_tracker"
            ]
        ...
        for layer_spec in self._graph_spec.input_layers:
            if initial_state is None:
                ...
                self._input_trackers[layer_spec.id_] = initial_state["input_trackers"][
                    layer_spec.id_
                ]
        ...
        for layer_spec in self._graph_spec.target_layers:
            if initial_state is None:
                self._output_trackers[layer_spec.id_] = self._get_output_stats_tracker(
                    layer_spec.datatype, layer_spec.feature_name
                )
            else:
                self._output_trackers[layer_spec.id_] = initial_state[
                    "output_trackers"
                ][layer_spec.id_]
    ...
    def _update_tracked_values(
        ...
        self._input_stats_tracker.update(
            id_to_feature=id_to_feature, sample_batch=inputs_batch
        )
        self._prediction_stats_tracker.update(
            id_to_feature=id_to_feature, sample_batch=predictions_batch
        )
        self._target_stats_tracker.update(
            id_to_feature=id_to_feature, sample_batch=targets_batch
        )
    ...
    def _get_mixpanel_pricing_metrics(self):
        ...
        trainable_params = int(
            np.sum([K.count_params(w) for w in self._training_model.trainable_weights])
        )
    ...
    def num_batches_completed_this_epoch(self):
        ...
        """
        return (
            self.num_training_batches_completed_this_epoch
            + self.num_validation_batches_completed_this_epoch
        )
    ...
    def _resolve_optimizer(self, training_settings):
        ...
        if optimizer == "SGD":
            ...
            return tf.keras.optimizers.RMSprop(
                learning_rate=learning_rate, centered=centered
            )
    ...
    def _get_dataset(self, partition, shuffle):
        ...
        dataset = self._data_loader.drop_dataset_index(indexed_dataset).batch(
            self.batch_size
        )
        return dataset
