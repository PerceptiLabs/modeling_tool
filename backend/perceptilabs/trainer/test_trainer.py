...
def df():
    df = pd.DataFrame(
        {"x1": [123.0, 24.0, 13.0, 45.0, -10.0], "y1": [1.0, 2.0, 3.0, 4.0, -1.0]}
    )
    yield df
...
def data_loader(df):
    settings = DatasetSettings(
        ...
        partitions=Partitions(
            training_ratio=4 / 5, validation_ratio=1 / 5, test_ratio=0.0
        ),
    )
...
def graph_spec():
    ...
    id3 = gsb.add_layer(
        "IoOutput", settings={"datatype": "numerical", "feature_name": "y1"}
    )
...
def graph_spec_faulty():
    ...
    id2 = gsb.add_layer("DeepLearningFC", settings={"n_neurons": 112})
    id3 = gsb.add_layer(
        "IoOutput", settings={"datatype": "numerical", "feature_name": "y1"}
    )
...
def test_progress_reaches_one(
    data_loader, training_model, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model, training_settings, training_session_id
    )
    ...
    trainer.run()
    assert (
        trainer.progress == 1.0 and trainer.num_epochs_completed == trainer.num_epochs
    )


def test_trainer_has_all_statuses(
    data_loader, training_model, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model, training_settings, training_session_id
...
def test_num_completed_batches_are_ok(
    data_loader, training_model, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model, training_settings, training_session_id
    ...
    assert (
        trainer.num_batches_per_epoch * trainer.num_epochs
        == trainer.num_batches_all_epochs
    )
    assert trainer.num_training_batches_completed_this_epoch > 0
    assert trainer.num_validation_batches_completed_this_epoch > 0
    assert (
        trainer.num_training_batches_completed_this_epoch
        + trainer.num_validation_batches_completed_this_epoch
        == trainer.num_batches_completed_this_epoch
    )
    assert trainer.num_batches_completed_this_epoch == trainer.num_batches_per_epoch


def test_num_completed_batches_are_ok_even_if_batch_size_is_larger_than_dataset(
    data_loader, training_model, training_settings, training_session_id
):
    batch_size = 200
    ...
    assert (
        data_loader.get_dataset_size(partition="training") > 0
        and data_loader.get_dataset_size(partition="training") < batch_size
    )
    assert (
        data_loader.get_dataset_size(partition="validation") > 0
        and data_loader.get_dataset_size(partition="validation") < batch_size
    )
...
def test_computed_results_do_not_change(
    data_loader, training_model, training_settings, training_session_id
):
    ...
    assert (
        pickled_initial_results != pickled_final_results
    )  # The results should be different
    assert (
        repickled_initial_results == pickled_initial_results
    )  # But the initial results shouldn't change.


def test_trainer_can_pause_and_unpause(
    data_loader, training_model, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model, training_settings, training_session_id
...
def test_trainer_can_pause_stop(
    data_loader, training_model, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model, training_settings, training_session_id
...
def test_trainer_custom_loss(
    data_loader, training_model, training_settings_custom_loss, training_session_id
):
...
def test_shuffle_is_called_for_training_but_not_for_validation(
    training_model, training_settings_shuffle_data, training_session_id
):
    data_loader = MagicMock()
    data_loader.get_dataset_size.return_value = 10

    trainer = Trainer(
        data_loader, training_model, training_settings_shuffle_data, training_session_id
    )

    step = trainer.run_stepwise()

    for i, _ in enumerate(step):
        _, kwargs = data_loader.get_dataset.call_args_list[
            i * 2
        ]  # Even calls are with training
        assert kwargs["partition"] == "training" and kwargs["shuffle"]

        _, kwargs = data_loader.get_dataset.call_args_list[
            i * 2 + 1
        ]  # Odd calls are with validation
        assert kwargs["partition"] == "validation" and (
            "shuffle" not in kwargs or not kwargs["shuffle"]
        )


def test_trainer_validate_raises_no_error(
    data_loader, training_model, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model, training_settings, training_session_id
...
def test_trainer_validate_raises_error_for_faulty_spec(
    data_loader, training_model_faulty, training_settings, training_session_id
):
    trainer = Trainer(
        data_loader, training_model_faulty, training_settings, training_session_id
    )
    ...
    ):  # Expects an error since num neurons == 112, while output shape == 1
        trainer.validate()


def test_trainer_calls_epoch_callback(
    graph_spec,
    data_loader,
    training_model,
    training_settings,
...
def test_trainer_calls_completed_callback(
    graph_spec,
    data_loader,
    training_model,
    training_settings,
...
def test_trainer_load_from_initial_gives_equal_results(
    graph_spec, data_loader, training_model, training_settings, training_session_id
):
    trainer1 = Trainer(
        data_loader, training_model, training_settings, training_session_id
    )
