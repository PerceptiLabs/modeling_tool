...
def test_zipfile_is_written_to_writable_path(
    export_type,
    message_broker,
    data_loader,
    graph_spec,
    ...
    step = interface.run_stepwise(
        CallContext(), data_loader, "123", "456", "789", "my_model"
    )
    next(step)
    ...
    actual_included_paths = set(
        model_archives_access.write.call_args[-1]["extra_paths"]
    )
    ...
    if export_type == "Standard":
        ...
        expected_included_paths = set.union(inference_model_paths, fastapi_paths)
...
def test_zipfile_has_model_json(
    rygg_mock,
    message_broker,
    data_loader,
    graph_spec,
    ...
    step = interface.run_stepwise(
        CallContext(), data_loader, "123", "456", serving_session_id, "my_model"
    )
    next(step)
...
def test_cleanup_after_ttl(
    message_broker, data_loader, graph_spec, temp_path, tensorflow_support_access
):
...
def test_serve_two_different_models(
    rygg_mock, message_broker, data_loader, graph_spec, temp_path
):
    results_access = ServingResultsAccess(rygg=rygg_mock)
    ...
    step_a = serve_model(
        serving_session_id=serving_session_a, export_settings={"Type": "Standard"}
    )
    next(step_a)

    serving_session_b = results_access.new_id({}, "123")
    step_b = serve_model(
        serving_session_id=serving_session_b, export_settings={"Type": "Compressed"}
    )
    next(step_b)
