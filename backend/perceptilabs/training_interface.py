...
class TrainingSessionInterface:
        ...
        try:
            ...
            with self._message_broker.subscription() as queue:
                yield from self._main_loop(
                    call_context, queue, trainer, results_interval, training_session_id
                )
    ...
    def _handle_error(self, call_context, e, training_session_id):
        ...
        self._results_access.store(
            call_context, training_session_id, {"error": error.to_dict()}
        )
        logger.exception("Exception in training session interface!")
        utils.send_ex_to_sentry(e, call_context)

    def _main_loop(
        self, call_context, queue, trainer, results_interval, training_session_id
    ):
    ...
    def _setup_trainer(
        ...
        if use_checkpoint:
            ...
            initial_state = self._epochs_access.load_state_dict(
                call_context, training_session_id, epoch_id
            )
        ...
        def on_training_started():
            tracking.send_training_started(
                self._event_tracker, call_context, model_id, graph_spec
            )

        def on_training_stopped(*args):
            tracking.send_training_stopped(
                self._event_tracker, call_context, model_id, graph_spec, *args
            )
    ...
    def _save_epoch(self, call_context, trainer):
        if (
            ...
            and trainer.num_training_batches_completed_this_epoch == 0
        ):
    ...
    def _check_for_slowdown(self, epoch, epoch_time, call_context):
        ...
        if (
            not self._has_reported_slowdown
            and slowdown_per_epoch > self._max_slowdown_rate
        ):
            logger.warning(
