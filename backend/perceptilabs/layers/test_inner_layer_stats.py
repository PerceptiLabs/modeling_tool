...
@pytest.fixture()
def outputs():
    return {"1": {"output": tf.constant([3.0])}}


@pytest.fixture()
def trainables_by_layer():
    return {"1": {"weights": tf.constant([3.0]), "bias": tf.constant([12.0])}}
...
def test_serialized_trackers_are_equal(
    inner_layers, outputs, trainables_by_layer, gradients_by_layer
):
...
def test_layer_weights_is_array(
    inner_layers, outputs, trainables_by_layer, gradients_by_layer
):
...
def test_layer_gradients_contain_exactly_one_float(
    inner_layers, outputs, trainables_by_layer, gradients_by_layer
):
    tracker = InnerLayersStatsTracker(inner_layers)
    tracker.update(
    ...
    assert (
        ...
        and len(minimum) == 1
        and isinstance(minimum[0], np.float32)
    )
    assert (
        isinstance(maximum, list)
        and len(maximum) == 1
        and isinstance(maximum[0], np.float32)
    )
    assert (
        isinstance(average, list)
        and len(average) == 1
        and isinstance(average[0], np.float32)
    )
