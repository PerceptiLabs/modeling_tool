...
@pytest.fixture()
def graph_spec(temp_path_100x1):
...
def test_save_checkpoint(script_factory, graph_spec, temp_path_100x1):
    ...
    training_layer.on_export(temp_path_100x1, mode='TFModel+checkpoint')
    assert any(x.startswith('model.ckpt') for x in os.listdir(temp_path_100x1))

def test_initial_weights_differ(script_factory, temp_path_100x1):
...
def test_load_checkpoint(script_factory, temp_path_100x1):
    inputs_path = os.path.join(temp_path_100x1, '100x1_inputs.npy')
    targets_path = os.path.join(temp_path_100x1, '100x1_outputs.npy')
    ...
    tl1.on_export(temp_path_100x1, mode='TFModel+checkpoint')
    #tf.reset_default_graph()
    ...
    graph_spec2 = make_graph_spec(
        inputs_path,
        targets_path,
        checkpoint_path=temp_path_100x1,
        learning_rate=0.0
    )        
    graph2 = graph_spec_to_core_graph(script_factory, graph_spec2)
