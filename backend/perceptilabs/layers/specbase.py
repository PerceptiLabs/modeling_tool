...
class LayerSpec(ABC, MyPydanticBaseModel):
    ...
    def field_names(self):
        ...
        if hasattr(
            self, "fields"
        ):  # Pydantic >= 1.8.0 does not have the fields property.
            return list(self.fields.keys())
        else:
            return list(dict(self))

    def compute_field_hash(
        self, ignore_forward_connections=True, prefer_custom_code=True
    ):
        ...
        if prefer_custom_code and self.custom_code is not None:
            ...
            for field_name in self.field_names:
                ...
                try:
                    ...
                    logger.exception(
                        f"Failed hashing field {field_name} with type {type(field_value)} in layer {self.id_} [{self.type_}]"
                    )
                    raise
            return hasher.hexdigest()
        ...
        def resolve_bw_cons(dict_):
            return tuple(
                LayerConnection(
                    ...
                    src_var=repl_smp(conn_spec["src_var"]),
                    dst_id=id_,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        dict_["backward_connections"] = [
            {"src_id": c.src_id, "src_var": c.src_var, "dst_var": c.dst_var}
            for c in self.backward_connections
        ]
        dict_["forward_connections"] = [
            {"dst_id": c.dst_id, "dst_var": c.dst_var, "src_var": c.src_var}
...
class InnerLayerSpec(LayerSpec):
    ...
    def should_show_errors(self):
        ...
        return (
            super().should_show_errors
            or len(self.forward_connections) > 0
            or len(self.backward_connections) > 0
        )
