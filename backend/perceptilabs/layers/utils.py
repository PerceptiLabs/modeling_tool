...
def resolve_checkpoint_path(specs):
    ...
    if "//" in ckpt_path:
        if platform.system() == "Windows":
            ...
            new_ckpt_path = (
                os.path.sep + ckpt_path.split(2 * os.path.sep)[1]
            )  # Sometimes frontend repeats the directory path. /<dir-path>//<dir-path>/model.ckpt-1
            logger.warning(
...
def resolve_tf1x_activation_name(specs):
    ...
    if activation not in table:
        ...
        logger.warning(
            f"layer {layer_id} specified activation {activation}, but it was not found in tf1x activations table. No activation will be used for this layer"
        )
