...
class DeepLearningRecurrentSpec(InnerLayerSpec, MixinDeepLearning):
    ...
    def _from_dict_internal(
        ...
        if "Properties" in dict_ and dict_["Properties"] is not None:
            params["n_neurons"] = (
                int(dict_["Properties"]["Neurons"])
                if dict_["Properties"]["Neurons"]
                else None
            )
            params["version"] = dict_["Properties"]["Version"]
            params["time_steps"] = (
                int(dict_["Properties"]["Time_steps"])
                if dict_["Properties"]["Time_steps"]
                else None
            )
            params["return_sequence"] = dict_["Properties"]["Return_sequence"]
            params["activation"] = (
                dict_["Properties"]["Activation_function"]
                if dict_["Properties"]["Activation_function"]
                else "None"
            )
