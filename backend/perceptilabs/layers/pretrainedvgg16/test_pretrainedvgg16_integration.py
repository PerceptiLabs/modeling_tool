...
@pytest.mark.parametrize("pooling", ["None", "avg", "max"])
def test_vgg16_output_doesnot_change_in_training_mode_with_training_argument(
    ...
    assert (
        y1["output"].numpy() == y2["output"].numpy()
    ).all()  # vgg doesn't has dropouts


@pytest.mark.parametrize("pooling", ["None", "avg", "max"])
def test_vgg16_output_doesnot_change_in_inference_mode_with_training_argument(
    script_factory, pooling
):
    layer_spec = PreTrainedVGG16Spec(
        id_="layer_id",
