...
@pytest.mark.parametrize("pooling", ["None", "avg", "max"])
def test_mobilenetv2_output_changes_in_training_mode_with_training_argument(
...
@pytest.mark.parametrize("pooling", ["None", "avg", "max"])
def test_mobilenetv2_output_doesnot_change_in_inference_mode_with_training_argument(
    script_factory, pooling
):
    layer_spec = PreTrainedMobileNetV2Spec(
        id_="layer_id",
