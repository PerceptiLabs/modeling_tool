...
class MaskOutputStats(OutputStats):
    def __init__(
        self, iou=None, predictions=None, targets=None, loss=None, inputs=None
    ):
        self._iou = iou
    ...
    def __eq__(self, other):
        return (
            ...
            and np.all(self.inputs == other.inputs)
        )
    ...
    def _get_arbitrary_sample(self, type_="prediction"):
        ...
        if type_ == "prediction":
            ...
            batch = list(batch.values())[
                0
            ].numpy()  # works only if there is one input layer
        sample = batch[-1]
        return sample
    ...
    def get_data_objects(self, view=None):
        ...
        axs.pcolormesh(
            np.squeeze(np.mean(input_image, axis=-1)), cmap=plt.get_cmap("gray")
        )
        ...
        data_obj_overlayed_image = create_data_object(
            [overlayed_image], subsample_ratio=1
        )
        ...
        data_objects = {
            ...
            "ViewBox": {"Output": create_data_object([target_value], ["mask"])},
        }
        if view:
    ...
    def _get_dataobj_iou(self):
        training_iou_over_steps = self._iou.get_iou_over_steps_in_latest_epoch(
            phase="training"
        )
        validation_iou_over_steps = self._iou.get_iou_over_steps_in_latest_epoch(
            phase="validation"
        )

        (
            training_iou_over_epochs,
            validation_iou_over_epochs,
        ) = self.get_iou_over_epochs()

        validation_iou_over_steps = (
            training_iou_over_steps + validation_iou_over_steps
        )  # The frontend plots the training iou last, so this gives the effect that the validation curve is a continuation of the training curve.
    ...
    def get_iou_over_epochs(self):
        ...
        """
        training_iou_over_epochs = self._iou.get_iou_over_epochs(phase="training")
        validation_iou_over_epochs = self._iou.get_iou_over_epochs(phase="validation")
...
class MaskOutputStatsTracker(TrainingStatsTracker):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.iou_tracker == other.iou_tracker
            and np.all(self.predictions == other.predictions)
            and np.all(self.targets == other.targets)
            and np.all(self.inputs == other.inputs)
        )
