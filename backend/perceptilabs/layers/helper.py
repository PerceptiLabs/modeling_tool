...
class LayerHelper:
    ...
    def get_code(
        ...
        if prepend_imports:
            (
                ...
                thirdparty,
                plabs,
            ) = self._script_factory.get_layer_import_statements(
        ...
        if print_code:
            print(
                f"{self._layer_spec.id_} [{self._layer_spec.type_}] code:\n"
    ...
    def get_line_count(self, preamble=None, prepend_imports=False, layer_code=True):
        code = self.get_code(
            preamble=preamble, prepend_imports=prepend_imports, layer_code=layer_code
        )
