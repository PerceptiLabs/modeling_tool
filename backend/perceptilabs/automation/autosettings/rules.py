...
class DeepLearningFcOutputShapeFromLabels(InferenceRule):
    ...
    def is_applicable(
        ...
        self._labels_shape = lw_results[self._labels_conn.src_id].out_shape[
            self._labels_conn.src_var
        ]
...
class DeepLearningConvDoubleFeatureMaps(InferenceRule):
    ...
    def apply(
        ...
        return layer_spec.clone(
            modified_params={"feature_maps": 2 * prev_conv.feature_maps}
        )
...
class DataDataShouldUseLazy(InferenceRule):
    ...
    def _estimate_ram_size(self, path):
        estimator = lambda path: 2 * os.path.getsize(
            path
        )  # Default to arbitrary safety margin
...
class ProcessReshape1DFromPrimeFactors(InferenceRule):
    ...
    def is_applicable(
        ...
        self._factorizations = self._get_factorizations(
            input_length, max_factors=self.MAX_DIM
        )  # Save this for later
    ...
    def apply(
        ...
        """A new layer will be created based on the builder configuration"""
        factorizations = [
            f if len(f) == 3 else f + [1] for f in self._factorizations
        ]  # Make them 3D
    ...
    def _get_factorizations(self, number, max_factors=None):
        ...
        def get_factorizations_recursive(
            number, current_factors, factorizations, max_factors
        ):
            ...
            for factor in range(1, number):
                ...
                if number % factor == 0:
                    get_factorizations_recursive(
                        number, current_factors + [factor], factorizations, max_factors
                    )
    ...
    json_network = {
        "Layers": {
            ...
            "3": {
                "Name": "reshape",
                "Type": "ProcessReshape",
                "Properties": {"Shape": [12, 1, 3], "Permutation": [0, 1, 2]},
            ...
            "3.2": {
                ...
                "Code": None,
                "backward_connections": [["3.1", "conv1"]],
