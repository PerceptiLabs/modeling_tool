...
class InferenceRule(ABC):
    ...
    def is_topologically_valid(
        self, graph_spec: GraphSpec, layer_spec: LayerSpec
    ) -> Tuple[bool, List[LayerSpec]]:
    ...
    def is_applicable(
        ...
        lw_results: Dict[str, LayerResults],
    ) -> bool:
...
class SettingsEngine:
    ...
    def run(self, graph_spec: GraphSpec, lw_results=None):
        ...
        for spec in graph_spec.nodes_by_id.values():
            if spec.visited:
                ...
                logger.info(
                    f"Skipping settings recommendations for layer {spec.id_} because it has been modified by the user"
                )

            if isinstance(spec, DummySpec):
                skip_ids.append(spec.id_)
                logger.info(
                    f"Cannot make settings recommendation for layer {spec.id_}: no spec object/builder found for type '{spec.type_}'"
                )

        rules = [rc() for rc in self._rule_classes]
        ordered_ids, rule_instances = self._get_ordered_layers(
            graph_spec, rules, skip_ids
        )
        ...
        for layer_id in ordered_ids:
            ...
            for rule in rule_instances[original_layer_spec.id_]:
                current_layer_spec = self._maybe_apply_rule(
                    rule, current_graph_spec, current_layer_spec, lw_results
                )
            ...
            if current_layer_spec != original_layer_spec:
                ...
                current_graph_dict[
                    current_layer_spec.id_
                ] = current_layer_spec.to_dict()
    ...
    def _maybe_apply_rule(
        self, rule, current_graph_spec, current_layer_spec, lw_results
    ):
        new_layer_spec = current_layer_spec  # Default
        try:
            if rule.is_applicable(current_graph_spec, current_layer_spec, lw_results):
                new_layer_spec = rule.apply(
                    current_graph_spec, current_layer_spec, lw_results
                )
                logger.info(
                    f"Autosettings: applied rule {rule.__class__.__name__} to {current_layer_spec}"
                )
        except:
            logger.exception(
                f"Autosettings: rule {rule.__class__.__name__} crashed unexpectedly when applied to layer {current_layer_spec.id_} [{current_layer_spec.type_}]"
            )
        finally:
    ...
    def _get_ordered_layers(self, graph_spec, rules, skip_ids):
        ...
        for id_ in graph_spec.node_ids:
            ...
            for rule_class in self._rule_classes:
                rule = rule_class()
                is_valid, dependencies = rule.is_topologically_valid(
                    graph_spec, layer_spec
                )

                if is_valid:
                    nx_graph.add_edges_from(
                        [
                            (dependency.id_, layer_spec.id_)
