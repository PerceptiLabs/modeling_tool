...
class ModelRecommender:
    ...
    def get_graph(self, feature_specs):
        ...
        num_inputs = len([feature_spec for _, feature_spec in feature_specs.items() if feature_spec.iotype == 'input'])
        num_outputs = len([feature_spec for _, feature_spec in feature_specs.items() if feature_spec.iotype == 'target'])
        target_datatypes = [feature_spec.datatype for _, feature_spec in feature_specs.items() if feature_spec.iotype == 'target']
    ...
    def get_single_input_single_output_network(self, feature_specs, builder):
        ...
        if input_datatype == 'image' and target_datatype == 'mask':
            SegmentationModel().build(builder, input_feature_name, target_feature_name, input_feature_spec, target_feature_spec, data_loader=self._data_loader)
    ...
    def _add_decoder(self, builder, feature_name, feature_spec):
        ...
        if feature_spec.datatype == 'numerical':
            ...
            return MaskDecoderBlueprint().build(builder, feature_name, feature_spec, data_loader=self._data_loader)
        else:
