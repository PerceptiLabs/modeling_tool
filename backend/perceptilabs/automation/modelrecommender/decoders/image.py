...
class ImageDecoderBlueprint(DecoderBlueprint):
    def build(
        self,
        ...
        num_feature_maps = target_shape[
            -1
        ]  # The number of feature maps should be equal to the number of channels
        n_neurons = np.prod(target_shape)

        id1 = builder.add_layer("DeepLearningFC", settings={"n_neurons": n_neurons})
        id2 = builder.add_layer(
            "ProcessReshape", settings={"shape": tuple(target_shape)}
        )
