...
def test_get_latest_require_checkpoint(access, training_session_id):
    epoch_id = access.get_latest(
        {}, training_session_id, require_checkpoint=True, require_trainer_state=False
    )
    assert epoch_id == 200
...
def test_get_latest_require_state(access, training_session_id):
    epoch_id = access.get_latest(
        {}, training_session_id, require_checkpoint=False, require_trainer_state=True
    )
    assert epoch_id == 100
...
def test_get_latest_require_both(access, training_session_id):
    epoch_id = access.get_latest(
        {}, training_session_id, require_checkpoint=True, require_trainer_state=True
    )
    assert epoch_id == 10
