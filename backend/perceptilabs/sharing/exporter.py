...
class Exporter:
    ...
    def _export_fastapi_service(
        self, path, include_preprocessing=True, include_postprocessing=True
    ):
        ...
        inference_model_paths = self._export_inference_model(
            path, model=model
        )  # TODO: not used!

        dataset = self._data_loader.get_dataset(
            partition="training", apply_pipelines="loader"
        ).batch(1)
        ...
        for partition in ["test", "validation", "training"]:
            df = self._data_loader.get_data_frame(partition=partition).head(
                10
            )  # keep N first rows
            if len(df) > 0:
                break
        ...
        fastapi_utils.render_fastapi_example_script(
            path, self._data_loader.feature_specs
        ),
    ...
    def _export_compressed_model(self, path):
        ...
        model = self.get_inference_model(
            include_preprocessing=False, include_postprocessing=False
        )
    ...
    def _export_quantized_model(self, path):
        ...
        def representative_data_gen():
            ...
            for input_value, _ in (
                self._data_loader.get_dataset(partition="training")
                .batch(1)
                .take(data_size)
            ):
    ...
    def get_inference_model(
        self, include_preprocessing=True, include_postprocessing=True
    ):
