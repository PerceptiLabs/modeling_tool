...
def _resolve_field(tensor, layer_spec, metadata):
    if layer_spec.datatype == "categorical":
        ...
        descr = (
            f"A list of {full_dtype} with any of the following values: "
            + ", ".join(str(x) for x in metadata["preprocessing"]["mapping"].keys())
        )
        ...
        descr = (
            "A matrix of floats with shape [batch_size, "
        ...
        raise NotImplementedError(
            f"Cannot resolve type string for datatype {layer_spec.datatype}"
        )
...
def _render_data_model_snippet(model, graph_spec, metadata):
    ...
    code += "\n"
    code += (
        "conlist_ = lambda type_, size: conlist(type_, min_items=size, max_items=size)"
    )
    code += "\n"
...
def render_fastapi_example_requirements(path):
    third_party_imports = ["pandas", "pillow", "numpy"]
