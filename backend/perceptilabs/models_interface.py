...
class ModelsInterface:
    ...
    def get_training_status(self, call_context, model_id, training_session_id):
        results_dict = self._training_results_access.get_latest(call_context, training_session_id)
    ...
    def get_training_results(self, call_context, model_id, training_session_id, type_, layer_id=None, view=None):
        results_dict = self._training_results_access.get_latest(call_context, training_session_id)
    ...
    def _export_internal(self, call_context, options, model_id, dataset_settings_dict, training_session_id, training_settings_dict, frontend_settings, graph_settings=None):
    ...
    def _export_while_training(self, call_context, options, model_id, training_session_id, graph_settings):
    ...
    def _export_after_training(self, call_context, options, model_id, dataset_settings_dict, training_session_id, graph_settings):
        if graph_settings:
    ...
    def get_layer_info(self, call_context, model_id, dataset_settings_dict, layer_id=None, graph_settings=None):
        try:
            if graph_settings:
                graph_spec = GraphSpec.from_dict(graph_settings)
            else:
    ...
    def get_previews(self, call_context, model_id, dataset_settings_dict, graph_settings=None):
    ...
    def get_model_recommendation(self, call_context, dataset_id, model_name, skipped_workspace, settings_dict, model_path):
        try:
            ...
            model_id = self._model_access.create(call_context, dataset_id, model_name, model_path)
            try:
    ...
    def _track_model_recommended(self, call_context, model_id, skipped_workspace, data_loader, graph_spec, settings_dict):
    ...
    def import_model(self, call_context, archive_path, dataset_id, model_name, model_path):
        ...
        dataset_settings_dict, graph_spec_dict, training_settings_dict = \
            importer.run(call_context, dataset_id, archive_path)

        model_id = self._model_access.create(call_context, dataset_id, model_name, model_path)
        try:
            self._model_access.save_graph(call_context, model_id, GraphSpec.from_dict(graph_spec_dict))
        except:
            logger.warning("save_graph raised an error")
