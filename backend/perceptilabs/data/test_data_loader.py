...
def test_splitting():
    ...
    partitions = Partitions(
        training_ratio=3 / 6, validation_ratio=2 / 6, test_ratio=1 / 6, randomized=False
    )
    dataset_settings = DatasetSettings(
        feature_specs=feature_specs, partitions=partitions
    )
    dl = DataLoader(df, dataset_settings)
...
def test_splitting_rows_are_preserved():  # I.e., check that columns arent shuffled differently
    ...
    validate_set(
        dl.get_dataset(partition="validation"), expected_rows=[[4.0, 4.0], [5.0, 5.0]]
    )
    validate_set(dl.get_dataset(partition="test"), expected_rows=[[6.0, 6.0]])
...
def test_splitting_rows_are_preserved_with_randomized_partitions():  # I.e., check that columns arent shuffled differently
    ...
    partitions = Partitions(
        training_ratio=3 / 6, validation_ratio=2 / 6, test_ratio=1 / 6, randomized=True
    )
    dataset_settings = DatasetSettings(
        feature_specs=feature_specs, partitions=partitions
    )
...
def test_randomized_partitioning_with_same_seed_are_equal():
    ...
    def datasets_are_equal(set1, set2):
        for (x1, y1), (x2, y2) in zip(set1, set2):
            equal = np.isclose(x1["x"].numpy(), x2["x"].numpy()) and np.isclose(
                y1["y"].numpy(), y2["y"].numpy()
            )
            if not equal:
                return False
        return True
    ...
    assert datasets_are_equal(
        dl1.get_dataset(partition="training"), dl2.get_dataset(partition="training")
    )
    assert datasets_are_equal(
        dl1.get_dataset(partition="validation"), dl2.get_dataset(partition="validation")
    )
    assert datasets_are_equal(
        dl1.get_dataset(partition="test"), dl2.get_dataset(partition="test")
    )
...
def test_randomized_partitioning_with_diff_seed_are_unequal():
    ...
    dataset_settings1 = DatasetSettings(
        feature_specs=feature_specs, partitions=partitions1
    )
    ...
    dataset_settings2 = DatasetSettings(
        feature_specs=feature_specs, partitions=partitions2
    )
...
def test_shuffle_gives_random_data():
    ...
    partitions = Partitions(randomized=True, seed=1234)
    dataset_settings = DatasetSettings(
        feature_specs=feature_specs, partitions=partitions
    )
    dl = DataLoader(df, dataset_settings)
...
def test_image_data_is_augmented_consistently(temp_path):
    ...
    feature_specs = {
        "x": FeatureSpec(
            ...
            preprocessing=ImagePreprocessingSpec(
                random_flip=True, random_flip_mode="horizontal", random_flip_seed=123
            ),
        ),
        "y": FeatureSpec(datatype="numerical", iotype="target"),
    }
    ...
    def datasets_are_equal(set1, set2):
        for (x1, y1), (x2, y2) in zip(set1, set2):
            equal = np.allclose(x1["x"].numpy(), x2["x"].numpy()) and np.allclose(
                y1["y"].numpy(), y2["y"].numpy()
            )
            if not equal:
                return False
...
def test_image_data_with_diff_seed_is_not_augmented_consistently(temp_path):
    ...
    fs2 = {
        "x": FeatureSpec(
            ...
            iotype="input",
            preprocessing=ImagePreprocessingSpec(
                random_flip=False, random_flip_mode="horizontal", random_flip_seed=100
            ),
        ),
        "y": FeatureSpec(datatype="numerical", iotype="target"),
    }
...
def test_image_data_is_augmented_consistently_for_repeated_calls(temp_path):
    ...
    assert datasets_are_equal(dl.get_dataset(), dl.get_dataset())


def test_image_data_is_augmented_consistently_for_repeated_calls_with_shuffle(
    temp_path,
):
    image_paths = []
    for _ in range(20):
    ...
    def datasets_are_equivalent(set1, set2):  # I.e., order doesn't matter
        dataset1 = {y["y"].numpy()[0]: x["x"].numpy() for x, y in set1}
        dataset2 = {y["y"].numpy()[0]: x["x"].numpy() for x, y in set2}
...
def test_image_data_resize_has_correct_shape(temp_path, ext):
    ...
    feature_specs = {
        "x": FeatureSpec(
            ...
            iotype="input",
            preprocessing=ImagePreprocessingSpec(
                resize=True, resize_mode="custom", resize_height=32, resize_width=8
            ),
        ),
        "y": FeatureSpec(datatype="numerical", iotype="target"),
    }
...
def test_get_dataframe_equals_original(random_partitions):
    ...
    data = {"x1": [x for x in range(n_rows)], "x2": [x**2 for x in range(n_rows)]}
    df = pd.DataFrame(data)
...
def test_get_dataframe_partition_matches_loader_partition(random_partitions):
    ...
    def datasets_are_equal(set1, set2):
        for (x1, y1), (x2, y2) in zip(set1, set2):
            equal = np.isclose(x1["x1"].numpy(), x2["x1"].numpy()) and np.isclose(
                y1["x2"].numpy(), y2["x2"].numpy()
            )
            if not equal:
                return False
        return True
    ...
    for partition in list(ratios.keys()) + ["all"]:
        ...
        partition_settings = DatasetSettings(
            feature_specs=feature_specs, partitions=Partitions(randomized=False)
        )
        partition_df = dl.get_data_frame(partition=partition)
