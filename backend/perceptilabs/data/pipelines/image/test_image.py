...
def test_build_from_metadata_gives_same_results(mnist_paths):
    ...
    metadata = {
        "loader": loader1.metadata,
        "augmenter": augmenter1.metadata,
        "preprocessing": preprocessing1.metadata,
    }
    loader2, augmenter2, preprocessing2, _ = ImagePipelineBuilder().load_from_metadata(
        preprocessing, metadata
    )

    dataset1 = (
        dataset.map(lambda x: loader1(x))
        .map(lambda x: augmenter1((0, x)))
        .map(lambda x: preprocessing1(x))
    )

    dataset2 = (
        dataset.map(lambda x: loader2(x))
