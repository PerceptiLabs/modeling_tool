...
@pytest.mark.parametrize(
    "shape_and_channels",
    [((32, 32, 1), 1), ((32, 32), 1), ((32, 32, 3), 3), ((32, 32, 4), 3)],
)
...
def test_image_data_is_loaded_with_custom_size(temp_path, ext):
    ...
    preprocessing = ImagePreprocessingSpec(
        resize=True,
        resize_mode="custom",
        resize_height=expected_height,
        resize_width=expected_width,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ImagePreprocessingSpec(
            resize=True, resize_mode="automatic", resize_automatic_mode="min"
        ),
        ImagePreprocessingSpec(
            resize=True, resize_mode="automatic", resize_automatic_mode="max"
        ),
        ImagePreprocessingSpec(
            resize=True, resize_mode="automatic", resize_automatic_mode="mode"
        ),
    ],
)
...
def test_image_data_is_loaded_with_automatic_shape_mean(temp_path, ext):
    ...
    preprocessing = ImagePreprocessingSpec(
        resize=True, resize_mode="automatic", resize_automatic_mode="mean"
    )
    loader = Loader.from_data(dataset, preprocessing=preprocessing)
