...
class Augmenter(BasePipeline):
    ...
    def _get_random_rotation(self):
        ...
        def random_rotation(x, index):
            scalar_angle = factor * tf.random.stateless_uniform(
                shape=(1,), seed=(seed, index)
            )
            x = tfa.image.rotate(
                x,
                angles=scalar_angle,
    ...
    def _get_random_crop(self, original_shape):
        ...
        if original_height and height > original_height:
            raise ValueError(
                f"Error in random cropping: Target height ({height}) cannot be greater than original height ({original_height})!"
            )
        if original_width and width > original_width:
            raise ValueError(
                f"Error in random cropping: Target width ({width}) cannot be greater than original width ({original_width})!"
            )

        def random_crop(x, index):
            y = tf.image.stateless_random_crop(
                value=x, size=(height, width, n_channels), seed=(seed, index)
            )
            return y
