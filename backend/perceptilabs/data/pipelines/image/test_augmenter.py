...
def test_horizontal_flip():
    ...
    augmenter = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            random_flip=True, random_flip_mode="horizontal", random_flip_seed=12
        )
    )
...
def test_vertical_flip():
    ...
    augmenter = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            random_flip=True, random_flip_mode="vertical", random_flip_seed=12
        )
    )
    actual_image = augmenter((0, original_image))
    assert (actual_image == expected_image).numpy().all()
...
def test_vertical_and_horizontal_flip():
    ...
    augmenter = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            random_flip=True, random_flip_mode="both", random_flip_seed=12
        )
    )
    actual_image = augmenter((0, original_image))
    assert (actual_image == expected_image).numpy().all()
...
def test_same_row_is_flipped_unequally_when_seeds_are_different():
    ...
    augmenter2 = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            random_flip=True, random_flip_mode="horizontal", random_flip_seed=21
        )
    )
...
def test_same_row_is_rotated_equally_when_seeds_are_equal():
    ...
    augmenter1 = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            ...
            random_rotation_factor=0.5,
            random_rotation_fill_mode="reflect",
            random_rotation_seed=12,
        )
    )
    augmenter2 = Augmenter(
        preprocessing=ImagePreprocessingSpec(
...
def test_random_crop_has_correct_shape():
    ...
    augmenter = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            random_crop=True,
            random_crop_height=16,
            random_crop_width=16,
...
def test_image_random_crop_raises_error_too_high():
    ...
    augmenter = Augmenter(
        preprocessing=ImagePreprocessingSpec(
            random_crop=True,
            random_crop_height=64,
            random_crop_width=16,
            random_crop_seed=12344,
        )
    )
