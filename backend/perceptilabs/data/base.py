...
def validate_pipeline_arg(arg):
    ...
    if arg not in valid_pipelines and arg is not None:
        raise ValueError(
            "Pipeline argument must be 'None' or one of: "
...
class DataLoader:
    ...
    def ensure_initialized(self):
        ...
        (
            ...
            self._test_set,
        ) = self._build_and_partition_data(
            self._data_frame, self._dataset_settings, self._num_repeats
        )
        if self._metadata is None:
            pipelines, self._metadata = self._build_pipelines(
                self._dataset_settings, self._training_set
            )
        else:
            pipelines = self._load_pipelines(self._dataset_settings, self._metadata)

        self._loader_pipelines = pipelines[
            "loader"
        ]  # TODO: create a namedtuple instead?
    ...
    def compute_metadata(
        cls, data_frame, dataset_settings, num_repeats=1, on_status_updated=None
    ):
        if data_frame is None:
            raise ValueError("Dataframe cannot be None!")
        ...
        _, metadata = cls._build_pipelines(
            dataset_settings, training_set, on_status_updated=on_status_updated
        )
        return metadata

    @classmethod
    def _build_and_partition_data(cls, data_frame, dataset_settings, num_repeats):
        cls._validate_feature_specs(
            data_frame.columns, dataset_settings.used_feature_specs
        )

        full_dataset, full_dataset_size = cls._create_indexed_dataset(
            data_frame, dataset_settings
        )
        ...
        repeated_training_set = cls._repeat_dataset(
            training_set,
            count=num_repeats,
            index_offset=full_dataset_size,  # TODO: num repeats should be a property of dataset_settings
        )
    ...
    def _repeat_dataset(original_dataset, count, index_offset=None):
        ...
        for _ in range(count - 1):
            current_dataset = current_dataset.concatenate(
                original_dataset.map(
                    lambda index, value: (index + index_offset, value),
        ...
        logger.info(
            f"Repeated dataset {count} times. Duration: {time.perf_counter() - t0:.3f}s"
        )
    ...
    def _load_pipelines(cls, dataset_settings, metadata):
        ...
        for feature_name, feature_spec in feature_specs.items():
            ...
            feature_metadata = (
                metadata["metadata"][feature_name]
                if "metadata" in metadata
                else metadata[feature_name]
            )
    ...
    def _build_pipelines(cls, dataset_settings, training_set, on_status_updated=None):
        ...
        for feature_name, feature_spec in feature_specs.items():
            ...
            logger.info(
                f"Built pipelines for {feature_name}. Duration: {time.perf_counter() - t0:.3f}s"
            )
        pipelines = {
    ...
    def _create_indexed_dataset(df, dataset_settings):
        ...
        indexed_dataset = tf.data.Dataset.zip(
            (indices, dataset)
        )  # This helps us map samples back to rows in the dataframe when debugging

        logger.info(
            f"Created indexed TensorFlow dataset. Duration: {time.perf_counter() - t0:.3f}s"
        )
    ...
    def _get_dataset_partition(self, partition):
        if partition == "training":
            ...
            return self._training_set.concatenate(self._validation_set).concatenate(
                self._test_set
            )
        else:
            raise ValueError(
                f"Invalid partition '{partition}'. Should be 'training', 'validation' or 'test'"
            )
    ...
    def drop_dataset_index(indexed_dataset):
        dataset = indexed_dataset.map(
            lambda index, inputs, targets: (inputs, targets),
            num_parallel_calls=tf.data.AUTOTUNE,
        )
    ...
    def _apply_pipelines(self, dataset, which="all"):
        ...
        logger.debug(
            f"Applied pipelines to dataset. Duration: {time.perf_counter() - t0:.3f}s"
        )
    ...
    def _split_inputs_and_targets(self, dataset):
        ...
        def func(indices, data):
            ...
            for (
                feature_name,
                feature_spec,
            ) in self._dataset_settings.used_feature_specs.items():
