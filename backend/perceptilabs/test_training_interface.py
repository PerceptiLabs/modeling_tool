...
def test_stopping_mid_training(
    ...
    assert (
        actual_progress == expected_progress < 1.0
    )  # Assert progress doesnt change after stopping


def test_pausing_mid_training(
    queue,
    message_broker,
    data_loader,
    graph_spec,
    ...
    assert counter > step_unpause and (
        step_unpause > step_pause
    )  # Assert we paused/unpaused
    assert status_list.count("Paused") == steps_paused


def test_export_mid_training(
    monkeypatch,
    queue,
    message_broker,
    data_loader,
...
@pytest.mark.parametrize(
    "model_id,session_id,expect_finished",
    [
...
def test_ignores_stopping_for_different_interface(
    model_id,
    session_id,
    expect_finished,
    queue,
...
def test_tf_memory_growth_enabled(
    message_broker, data_loader, graph_spec, training_model, training_settings
):
    model_access = MagicMock()
    epochs_access = MagicMock()
    results_access = MagicMock()
...
def test_slowdown_is_detected(
    ...
    monkeypatch.setattr(
        perceptilabs.utils, "send_message_to_sentry", fake_sentry_call, raising=True
    )
