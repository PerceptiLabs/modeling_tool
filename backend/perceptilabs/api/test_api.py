...
def rygg_mock(monkeypatch, tmp_path):
    ...
    def get_tf_hub_cache_dir(self, call_context):
        response = {"tf_hub_cache_dir": str(tmp_path.stem)}
        return response
    ...
    def create_model(self, call_context, dataset_id, model_name, location=None):
        ...
        models[model_id] = {"meta": meta, "model_json": {}}
        return meta
...
def dataset_settings():
    dict_ = {
        "featureSpecs": {
            "x1": {"iotype": "input", "datatype": "numerical", "preprocessing": {}},
            "x2": {
                "iotype": "do not use",
                "datatype": "numerical",
                "preprocessing": {},
            },
            "y1": {"iotype": "target", "datatype": "categorical", "preprocessing": {}},
        },
        "partitions": [70, 20, 10],
...
def assert_export(
    ...
    assert has_been_called_with(
        mixpanel_mock.track,
        kwargs={
            "distinct_id": "anton.k@perceptilabs.com",
            "event_name": "model-exported",
        },
    )
...
def assert_info(client, model_id, dataset_settings, graph_settings=None):
    res = client.post(
        ...
        json={"datasetSettings": dataset_settings, "graphSettings": graph_settings},
    )
...
def assert_testing(
    client, mixpanel_mock, model_id, training_session_id, dataset_settings
):
    res = client.post(
        f"/inference/testing",
        json={
...
def test_modeling_flow_basic(
    client, deployment, mixpanel_mock, tmp_path, dataset_settings, training_settings
):
    ...
    training_session_id = assert_training(
        client, mixpanel_mock, model_id, dataset_settings, training_settings
    )
...
def test_modeling_flow_with_graph_settings_in_payload(
    client, deployment, mixpanel_mock, tmp_path, dataset_settings, training_settings
):
    assert_data(client, dataset_settings)
...
@pytest.mark.parametrize("client", ["threaded", "celery"], indirect=True)
def test_multi_input_modeling_flow(
    client, mixpanel_mock, tmp_path, dataset_settings, training_settings
):
    ...
    assert_export(
        client, mixpanel_mock, dataset_settings, model_id, training_session_id, tmp_path
    )


@pytest.mark.parametrize("client", ["threaded", "celery"], indirect=True)
def test_modeling_flow_stop_training(
    client, tmp_path, dataset_settings, training_settings, mixpanel_mock
):
...
@pytest.mark.parametrize("client", ["threaded", "celery"], indirect=True)
def test_error_preprocessing(
    monkeypatch, client, tmp_path, dataset_settings, training_settings
):
...
@pytest.mark.skipif(is_windows(), reason="flaky")
@pytest.mark.parametrize("client", ["threaded", "celery"], indirect=True)
def test_error_training(
    monkeypatch, client, tmp_path, dataset_settings, training_settings, mixpanel_mock
):
    assert_data(client, dataset_settings)
    model_id, _ = assert_model_recommendation(client, mixpanel_mock, dataset_settings)
...
@pytest.mark.parametrize(
    "auth_token", [{"this": "is the decoded token", "sub": "a123"}]
)
...
def test_sharing_flow(
    ...
    assert_training(
        ...
        mixpanel_mock,
        res.json["modelId"],
        res.json["datasetSettings"],
        res.json[
            "trainingSettings"
        ],  # TODO: these should NOT be returned. We should instead return the dataset settings ID (hash? or model id?) and training session ID (model id)... What about frontend settings? Leave for now?
    )
