...
def aggregate_summaries(all_summaries):
    ...
    for metric, values in metric_values.items():
        aggregated_values["metric_" + metric + "_max_over_layers"] = float(
            np.max(values)
        )
        aggregated_values["metric_" + metric + "_avg_over_layers"] = float(
            np.average(values)
        )
        aggregated_values["metric_" + metric + "_min_over_layers"] = float(
            np.min(values)
        )
        aggregated_values["metric_" + metric + "_num_layers"] = len(
            values
        )  # Number of layers with the given metric
