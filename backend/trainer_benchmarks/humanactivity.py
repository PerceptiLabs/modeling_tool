...
class HumanActivitySuite(BenchmarkSuite):
    def __init__(self):
        self.data_loader = data_utils.get_humanactivity_loader()
    ...
    def _get_trainer_results(self, trainer, training_settings, training_duration):
        ...
        """
        stats = next(
            iter(trainer.get_output_stats().values())
        )  # This dataset has a single output
        results = stats.get_summary()
        results["training_duration (s)"] = training_duration
        results["epoch_duration (ms)"] = (
            training_duration / int(training_settings["Epochs"]) * 1000
        )
    ...
    def _run_custom_model(self):
        path = os.path.join(
            data_utils.get_tutorial_data_directory(), "HumanActivity", "data.csv"
        )
        builder = GraphSpecBuilder()

        c1 = builder.add_layer(
            "IoInput",
            settings={"datatype": "image", "feature_name": "images", "file_path": path},
        )
        c2 = builder.add_layer(
            "DeepLearningConv",
            settings={
                "conv_type": "Conv",
                "conv_dim": "2D",
        ...
        c6 = builder.add_layer(
            "DeepLearningFC", settings={"n_neurons": 6, "activation": "Softmax"}
        )
        c7 = builder.add_layer(
            "IoOutput",
            settings={
