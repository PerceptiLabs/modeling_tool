...
class MnistSuite(BenchmarkSuite):
    ...
    def _run_baseline_model(self):
        ...
        training_set = (
            self.data_loader.get_dataset(partition="training")
            .map(convert_dataset)
            .shuffle(buffer_size=dataset_size)
            .batch(32)
        )
        validation_set = (
            self.data_loader.get_dataset(partition="validation")
            .map(convert_dataset)
            .batch(32)
        )
        ...
        history = model.fit(
            training_set, epochs=n_epochs, validation_data=validation_set
        )
