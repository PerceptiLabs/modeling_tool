<template lang="pug">
nav.app-header_nav(v-hotkey="keymap")
  ul.header-nav
    li.header-nav_item(v-for="(item, i) in navMenu", :key="i")
      ...
      ul.header-nav_sublist.sublist--top(
        ...
        li.header-nav_item(
          ...
          .separator(v-if="subItem.type === 'separator'")
          button.btn.btn--link.header-nav_sublist-btn(
            type="button",
            ...
            span.text-disable.hotkey(v-if="subItem.accelerator") {{ subItem.accelerator }}
...
export default {
  ...
  computed: {
      ...
      this.navMenu.forEach(item => {
        item.submenu.forEach(subItem => {
          if (subItem.accelerator) {
            this.dataKeymap[subItem.accelerator] = ev =>
              subItem.active(ev, true);
          }
        });
      });
    ...
    hasNetworkWithUnsavedChanges() {
      return (
        this.$store.getters["mod_workspace-changes/get_networksWithChanges"]
          .length > 0
      );
    },
    isCurrentModelOpened() {
      return (
        this.currentModel &&
        ((this.viewType === "model" &&
          this.currentModel.networkMeta.hideModel === false) ||
          (this.viewType === "statistic" &&
            this.currentModel.networkMeta.openStatistics === true &&
            this.currentModel.networkMeta.hideStatistics === false))
      );
    },
  },
  methods: {
      ...
      return [
        ...
        },
        {
          ...
          submenu: [
            ...
            {
              label: "Save",
              accelerator: this.isMac ? "meta+s" : "ctrl+s",
              enabled: this.openApp,
              active: this.saveModel,
            },
            {
              label: "Save as",
              accelerator: this.isMac ? "meta+shift+s" : "ctrl+shift+s",
              enabled: this.openApp,
              active: this.saveModelAs,
            },
            { type: "separator" },
            {
        ...
        },
        {
          ...
          submenu: [
            {
              label: "Undo",
              accelerator: this.isMac ? "meta+z" : "ctrl+z",
              role: "undo",
              enabled: this.openApp,
              active: this.toPrevStepHistory,
            },
            {
              label: "Redo",
              accelerator: this.isMac ? "meta+shift+z" : "ctrl+y",
              role: "redo",
              enabled: this.openApp,
              active: this.toNextStepHistory,
            },
            { type: "separator" },

            {
              label: "Cut",
              accelerator: this.isMac ? "meta+x" : "ctrl+x",
              role: "cut",
              enabled: this.openApp,
              active: this.HCCut,
            },
            {
              label: "Copy",
              accelerator: this.isMac ? "meta+c" : "ctrl+c",
              role: "copy",
              enabled: this.openApp,
              active: this.HCCopy,
            },
            {
              label: "Paste",
              accelerator: this.isMac ? "meta+v" : "ctrl+v",
              role: "paste",
              enabled: this.openApp,
              active: this.onPaste,
            },
            ...
            {
              label: "Select all",
              accelerator: this.isMac ? "meta+a" : "ctrl+a",
              role: "selectAll",
              enabled: this.openApp,
              active: this.HCSelectAll,
            },
            {
              label: "Deselect all",
              accelerator: this.isMac ? "meta+shift+a" : "ctrl+shift+a",
              enabled: this.openApp,
              active: this.HCDeselectAll,
            },
          ],
        },
        {
        ...
        },
        {
          ...
          submenu: [
            ...
            {
              label: "Delete",
              accelerator: this.isMac ? "meta+backspace" : "delete",
              active: this.HC_delete,
              visible: false,
            },
            {
              label: "DeleteMac",
              accelerator: this.isMac ? "backspace" : "",
              active: this.HC_delete,
              visible: false,
            },
