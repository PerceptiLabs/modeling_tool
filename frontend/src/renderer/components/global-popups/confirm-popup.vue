...
base-global-popup(
  ...
  template(slot="action")
    button.btn.btn--secondary(type="button", @click="cancel") Cancel
    button.btn(
      :class="{ 'btn--primary': isDefaultConfirmType, 'btn--danger': isDangerConfirmType }",
      type="button",
      @click="ok"
    ) Ok
</template>
...
export default {
  ...
  computed: {
    ...
    isDefaultConfirmType() {
      return this.$store.state.globalView.popupConfirmType === "DEFAULT";
    },
    isDangerConfirmType() {
      return this.$store.state.globalView.popupConfirmType === "DANGER";
    },
  },
  methods: {
    ok() {
