...
export default {
  ...
  mounted() {
    this.$refs.rootBaseElement.addEventListener(
      "touchstart",
      this.switchMousedownEvent
    );
  },
  ...
  beforeDestroy() {
    if (this.$refs.rootBaseElement) {
      ...
      this.$refs.rootBaseElement.removeEventListener(
        "touchstart",
        this.switchMousedownEvent
      );
      /*appMode*/
      ...
      this.$refs.rootBaseElement.removeEventListener(
        "mouseup",
        this.Mix_paintArrow_arrowEndPaint
      );
  ...
  computed: {
    ...
    classEl() {
      return {
        ...
        "net-element--active":
          this.isSelectedEl || this.dataEl.layerId === this.closestElId,
        "element--hidden":
          this.dataEl.layerMeta && this.dataEl.layerMeta.isInvisible
      };
    },
    classElWindow() {
      return {
  ...
  watch: {
    "modAddComponentState.draggedComponnentPosition": {
      handler(data) {
        const {
          isShiftPressed,
          isFirstComponentDragged,
          draggedComponnentPosition: { x, y }
        } = this.modAddComponentState;
        ...
        if (isShiftPressed && isFirstComponentDragged) {
          Object.keys(this.networkElmentPositions).map(elId => {
            const elPositionX =
              calcLayerPosition(
                this.networkElmentPositions[elId].left,
                this.wsZoom
              ) +
              75 * this.wsZoom;
            const elPositionY =
              calcLayerPosition(
                this.networkElmentPositions[elId].top,
                this.wsZoom
              ) +
              20 * this.wsZoom;
            ...
            if (
              ...
              (distance === null &&
                currentDistance <= SHIFT_HOLDING_CONNECT_COMPONENT_MAX_DISTANCE)
            ) {
        ...
        this.$store.dispatch(
          "mod_addComponent/setClosestElementId",
          closestElId
        );
      },
      deep: true,
  ...
  methods: {
    ...
    switchClickEvent(ev) {
      ...
      if (!this.editIsOpen && !this.layerContainer) {
        this.$store.commit(
          "mod_statistics/CHANGE_viewBoxSelectElArr",
          this.dataEl
        );
    ...
    calcWindowPosition(el) {
      ...
      let winCenterWidth =
        windowWs.scrollLeft + (windowWsWidth - layerHeight) / 2;
      let winCenterHeight =
        windowWs.scrollTop + (windowWsHeight - layerHeight) / 2;
