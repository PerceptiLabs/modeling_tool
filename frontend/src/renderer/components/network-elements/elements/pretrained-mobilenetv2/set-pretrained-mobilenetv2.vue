...
div
  .settings-layer_section
    .form_row
      ...
      .form_input
        base-radio(
          group-name="include_top",
          :value-input="true",
          v-model="settings.include_top"
        )
          span Yes
        base-radio(
          group-name="include_top",
    ...
    .form_row
      ...
      .form_input
        base-radio(
          group-name="weights",
          value-input="None",
          v-model="settings.weights"
        )
          span None
        base-radio(
          group-name="weights",
          value-input="imagenet",
          v-model="settings.weights"
        )
          span imagenet
</template>
...
export default {
  ...
  methods: {
    saveSettings(tabName) {
      if (this.settings.alpha < 0 || this.settings.alpha > 1) {
        return;
      }
