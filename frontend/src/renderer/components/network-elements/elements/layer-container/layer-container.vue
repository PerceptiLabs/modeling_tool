...
.layer-container(v-if="showChildContainer")
  ...
  template(v-else)
    .layer-container_box(:style="containerStyle")
      h4.layer-container_title(:style="zoomingStyle") {{ elementData.layerName }}
...
export default {
  ...
  computed: {
    ...
    containerStyle() {
      ...
      let acumulator = collectPositionRecursion(
        this.elementData.containerLayersList,
        []
      );
      ...
      if (containerHaveOpenSubContainer) padding += 30;
  ...
  methods: {
    ...
    toggleContainer(val) {
      this.$store.dispatch("mod_workspace/TOGGLE_container", {
        val,
        container: this.elementData
      });
    },
    closeContext() {
      this.contextIsOpen = false;
...
.layer-container_title {
  ...
  right: 0;
  margin-bottom: 1rem;
  background-color: rgba($bg-workspace, 0.5);
