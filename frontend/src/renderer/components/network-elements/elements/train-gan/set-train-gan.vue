    ...
    .form_row
      ...
      #tutorial_stop-condition.tutorial-relative.form_input(
        ...
        base-radio(
          group-name="group2",
          value-input="Epochs",
          v-model="settings.Stop_condition"
        )
          span None
        base-radio(
          group-name="group2",
          value-input="TargetAccuracy",
          v-model="settings.Stop_condition"
        )
...
export default {
  ...
  beforeMount() {
    ...
    for (let key in elList) {
      ...
      if (
        elList[key].layerType === "Other" &&
        elList[key].componentName === "MathSwitch"
      ) {
    ...
    if (!this.settings.switch_layer && this.allSwitchLayers.length)
      this.settings.switch_layer = this.allSwitchLayers[0].value.toString();
    if (!this.settings.real_data_layer && this.allRealDataLayers.length)
      this.settings.real_data_layer = this.allRealDataLayers[0].value.toString();
  },
  data() {
    return {
      allSwitchLayers: [],
