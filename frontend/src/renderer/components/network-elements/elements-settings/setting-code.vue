<template lang="pug">
.settings-layer_section(:class="{ 'code_full-view': fullView }")
  .bookmark_head
    ul.bookmark_tab-list
      ...
      button.bookmark_tab.bookmark_tab--error(
        type="button",
        v-if="errorData",
    ...
    button.btn.btn--link.icon.icon-full-screen-code(
      type="button",
...
export default {
  ...
  mounted() {
    ...
    if (!this.currentEl.layerCode) {
      ...
      typeof this.currentEl.layerCode === "object" &&
      !this.currentEl.layerCode.Output
    ) {
  ...
  methods: {
    getCode() {
      ...
      this.$store.dispatch("mod_api/API_getCode", value).then(code => {
        this.setCode(code);
      });
    },
    setCode(objCode) {
    ...
    toggleFullView() {
      ...
      document
        .querySelector(".popup_body")
        .classList.toggle("popup_body--show-code");
