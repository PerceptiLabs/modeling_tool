...
export default {
  ...
  methods: {
    ...
    calcLayerItemStyle(elementName, backgroundColor) {
      const bgc =
        this.hoveredElement === elementName ? backgroundColor : "#23252A";
    ...
    startComponentPositionUpdates(event) {
      ...
      this.$store.dispatch("mod_addComponent/setDraggedComponnentPosition", {
        x: xPosition,
        y: yPosition,
      });
    ...
    stopComponentPositionUpdates(event) {
      ...
      if (
        ...
        if (isShiftPressed) {
          if (this.previousAddedElementId !== null) {
            // is second element placed by holding shift key

            this.$store.dispatch("mod_workspace/ADD_element", {
              event: fakeEvent,
            });
    ...
    cloneElement(elementName) {
      ...
      if (Array.isArray(this.$refs[this.clickedElementName])) {
        this.clonedElement =
          this.$refs[this.clickedElementName][0].$el.cloneNode(true);
      } else {
        this.clonedElement =
          this.$refs[this.clickedElementName].$el.cloneNode(true);
      }
    },
    ...
    handleEscKeypress(event) {
      if (this.clickedElementName && event.key === "Escape") {
