...
import LayerObjectDetectionModel from "@/components/network-elements/elements/object-detection-model/viewBox-object-detection-model.vue";
...
export default {
  ...
  computed: {
    ...
    selectedMetric() {
      return this.$store.getters["mod_statistics/getSelectedMetric"](
        this.tabsKeyToBeChanged,
      );
    },
  },
  watch: {
  ...
  methods: {
    setBtnList(tabList) {
      ...
      if (this.sectionTitle === "ViewBox")
        this.$store.commit(
          "mod_statistics/setDefaultMetric",
          this.tabsKeyToBeChanged,
        );
    },
    resetBtnInfo() {
    ...
    layerMetrics() {
      return this.$store.getters["mod_statistics/getLayerMetrics"](
        this.tabsKeyToBeChanged,
      );
    },
  },
