...
def test_new_model_has_extant_location(rest, tmp_project, to_local_translator):
    ...
    assert_eventually(
        lambda: not os.path.exists(model.location), stop_max_delay=1000, wait_fixed=100
    )
...
def test_new_model_rejects_location(rest, tmpdir, tmp_project):
    with pytest.raises(Exception):
        with ModelClient.make(
            rest, name="test project", project=tmp_project.id, location=str(tmpdir)
        ) as model:
            pass
...
def test_next_name_works(rest, tmp_project):
    ...
    assert (
        ModelClient.get_next_name(rest, tmp_project.id, "this_is_a_prefix")["next_name"]
        == "this_is_a_prefix 1"
    )
...
def test_user_home_used(rest, tmp_project):
    ...
    try:
        with ModelClient.make(
            rest, name="test project", project=tmp_project.id, location=dest
        ) as model:
            model.save_json({})
