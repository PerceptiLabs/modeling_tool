...
class Project(SoftDeletableModel):
    ...
    def get_default(owner):
        ret = Project.available_objects.filter(name=DEFAULT_PROJECT_NAME, owner__in=[owner, Project.GRANDFATHERED_OWNER]).first()

        if not ret:
            os.makedirs(DEFAULT_PROJECT_DIR, exist_ok=True)
            ret = Project(name=DEFAULT_PROJECT_NAME, default_directory=DEFAULT_PROJECT_DIR, owner=owner)
            ret.save()
        return ret
