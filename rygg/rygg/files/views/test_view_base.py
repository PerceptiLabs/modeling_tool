...
class ViewTestBase(APITestCase):
    ...
    def call_and_expect_json(
        self, verb, url_path, expected_body, path="the_path", project_id="a_project_id"
    ):
    ...
    def post(
        self, url_path, data, path="the_path", project_id="a_project_id", **other_params
    ):
        with self.mock_get_path_param(path=path, project_id=project_id, **other_params):
            with url_with_token(
                url_path, path=path, project_id=project_id, **other_params
            ) as url:
