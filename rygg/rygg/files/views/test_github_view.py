...
class GithubImportTestCase(TempFileTester, ViewTestBase):
    ...
    def test_simple_case(self, translate_mock, import_repo_mock):
        ...
        import_repo_mock.assert_called_once_with(
            self.test_dir, "http://noop", overwrite=False
        )
...
class GithubExportTestCase(TempFileTester, ViewTestBase):
    def test_basic(self, translate_mock):
        ...
        with patch(
            "rygg.files.models.github.export_repo_basic",
            return_value=["2341234", "the_url"],
        ) as export_mock:

            response = self.post(
                "/github/export", request, path=self.test_dir, project_id=PROJECT_ID
            )
    ...
    def test_advanced(self, translate_mock):
        ...
        translate_mock.return_value = "translated_path"
        with patch(
            "rygg.files.models.github.export_repo_advanced",
            return_value=["2341234", "the_url"],
        ) as export_mock:
...
class GithubIssueTests(ViewTestBase):
    def test_simple(self):
        ...
        with patch(
            "rygg.files.models.github.create_issue", return_value="the issue number"
        ):
