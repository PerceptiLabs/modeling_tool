...
class RepoExporterAPI:
    ...
    def check_file_sizes(files: dict):
        ...
        if too_big:
            raise UserError(
                f"This directory contains files that are too large to export to GitHub. The limit is {GITHUB_MAX_MB}MB."
            )
    ...
    def add_files(self, files: dict, commit_message):
        ...
        element_list = [
            self._element_from_file_data(repo, file_path, data)
            for file_path, data in lazy_path_data_pairs
        ]
        return [
            self._add_elements_to_new_commit(repo, element_list, commit_message),
            self._get_repo_url(),
        ]
    ...
    def _get_repo_url(self):
        return GITHUB_URL_FORMAT.format(
            username=self.git.get_user().login, repo_name=self.repo_name
        )
