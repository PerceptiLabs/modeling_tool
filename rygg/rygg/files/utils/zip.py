...
def _unzipped_files_from_zipfile(zipfile_name, dest=None, cancel_token=Event()):
    ...
    return PresizedIterator(len(files), extracted_files_list())
...
def _get_items_from_unzip(
    zipfile: str, dest: Optional[str] = None, cancel_token=Event()
) -> Iterable[str]:
...
def _unzipped_files_from_unzip(
    zipfile: str, dest: Optional[str] = None, cancel_token=Event()
) -> Iterable[str]:
...
def _copied_files_from_unzip(zipfile, dest=None, cancel_token=Event()):
    ...
    def inner():
        with TemporaryDirectory() as tmp:
            ...
            unzipped = _get_items_from_unzip(
                zipfile, dest=tmp, cancel_token=cancel_token
            )
    ...
    return PresizedIterator(
        _get_expected_from_unzip(zipfile, cancel_token=cancel_token), inner()
    )
...
def _get_unzipper():
    def check_unzip_version():
        ...
        if not ok:
            raise Exception(
                f"Wrong version of unzip. Expected {SUPPORTED_VERSION}. Got version string '{first_line}'"
            )
