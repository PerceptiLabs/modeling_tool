...
class TranslatePathFromUser(TempFileTester, TestCase):
    def test_simple(self):
        cases = [
            ...
            [
                "Local from home path",
                "local",
                "~/a/x.txt",
                123,
                os.path.join(Path.home(), "a", "x.txt"),
            ],
            [
                "Local relative path",
                "local",
                "a/b.txt",
                123,
                os.path.join(Path.home(), "a", "b.txt"),
            ],
            [
                "Enterprise absolute path outside allowed",
                "enterprise",
                "/a",
                123,
                Exception,
            ...
            [
                "Enterprise absolute path inside allowed",
                "enterprise",
                os.path.join(self.test_dir, "123", "a.txt"),
                123,
                os.path.join(self.test_dir, "123", "a.txt"),
            ],
            [
                "Enterprise relative path",
                "enterprise",
                "a/b.txt",
                123,
                os.path.join(self.test_dir, "123", "a", "b.txt"),
            ],
        ]
        ...
        for name, ent_loc, path, project_id, expected in cases:
            ...
            with self.subTest(msg=name, path=path, project_id=project_id), patch(
                f"{TARGET}.IS_CONTAINERIZED", is_enterprise
                ...
                if isinstance(expected, str):
                    ...
                    self.assertRaises(
                        expected, target.translate_path_from_user, path, project_id
                    )
                else:
                    assert False
