...
def track(request):
    ...
    try:
        mp_response = get(
            url=MIXPANEL_API_JS_ENDPOINT + MIXPANEL_TRACK_PATH, params=params
        )
    except:
...
def decide(request):
    ...
    try:
        mp_response = get(
            url=MIXPANEL_API_JS_ENDPOINT + MIXPANEL_DECIDE_PATH, params=dataDict
        )
    except:
        logger.error("Error when sending tracking data to MixPanel")
...
def engage(request):
    ...
    try:
        mp_response = get(
            url=MIXPANEL_API_JS_ENDPOINT + MIXPANEL_ENGAGE_PATH, params=params
        )
    except:
